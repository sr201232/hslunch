...
bool runDisplayChecks() {
  ...
  unsigned passed = 0;
  if (duration(0,119) == 2000 && duration(8,128) == 10000) ++passed;
  if (duration(2,72) == 1000 && duration(4,120) == 1000 && duration(6,72) == 1000) ++passed;
  if (x(3,456,0) == 0 && x(3,456,999) == 0 && x(3,456,1000) == 0) ++passed;
  if (x(3,456,1040) == -1 && x(3,456,2000) == -42) ++passed;
  if (duration(3,456) == 6762 && x(3,456,5762) + 456 == 256 && x(3,456,9999) == -200) ++passed;
  ...
  if (speed(3)==42 && speed(5)==42 && speed(1)==42 && speed(8)==42) ++passed;
  if (adX(752,0)==0 && adX(372,5000)==-210 && adX(372,10000)==0 && duration(8,752)==10000) ++passed;
  auto parts = splitOriginPages("국내산: 쌀, 배추, 돼지고기, 닭고기, 오리고기 / 중국산: 김치",256);
  bool bounded = parts.size()>1; String all;
  for (const auto &part : parts) { bounded &= textWidth(part)<=256; all += part; }
  if (bounded && all.indexOf("오리고기")>=0 && all.indexOf("중국산: 김치")>=0) ++passed;
  if (originWidthLimit(2000)==802 && originWidthLimit(400)==400 && originWidthLimit(80)==256) ++passed;
