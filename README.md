# HS Lunch — 상암고 급식정보 전광판

서울 상암고등학교의 중식·알레르기 번호·원산지·칼로리를 나이스에서 받아 ESP32와 HUB75 LED 전광판에 한 줄로 표시하는 개인 프로젝트입니다.

**제작: 30805 김아준 · 프로젝트 시작: 2026.8.27(목)**
기존 프로젝트 안내: https://ajmk.kr/4

현재 버전: **1.4.0**. 코드 컴파일과 모의 화면 검증 결과는 `verification.md`를 참고하세요. 실물 패널의 해상도·스캔 방식과 배선을 먼저 확인해야 합니다.

`sangam_display/sangam_display.ino`를 Arduino IDE에서 엽니다. 같은 폴더의 헤더 파일도 필요합니다.
...
| 페이지 | 내용 | 표시 시간/이동 |
|---|---|---|
| 1 | 현재 한국 날짜·시간 `2026.08.08 11:57` 형식 | 2초 고정, `:`만 켜짐/꺼짐 3회 |
| 2 | `급식정보 전광판 프로젝트 by 30805 김아준, github.com/sr201232/hslunch` | 긴 문장 스크롤 규칙 |
| 3 | `급식 정보` | 1초 고정 |
| 4 | 메뉴 + 음식별 알레르기 번호 | 긴 문장 스크롤 규칙 |
| 5 | `원산지 정보` | 1초 고정 |
| 6 | 원산지별 식재료 목록 중 이번 구간 | 초당 42픽셀, 최대 15초 및 메뉴 시간 이하 |
| 7 | `칼로리 정보` | 1초 고정 |
| 8 | 나이스 칼로리 값 | 짧으면 2초 고정 |
| 9 | `지성: 지극과 정성 / 상암고등학교` | 10초 동안 초당 42픽셀로 반복 스크롤 후 1페이지로 즉시 전환 |

1→2부터 7→8까지는 이전 문장이 위로 나가고 다음 문장이 아래에서 올라옵니다(전환 0.4초). **8→9와 9→1은 즉시 전환**합니다. 표의 표시 시간에는 전환 시간이 포함되지 않습니다.

긴 문장은 화면 왼쪽부터 보이는 상태에서 **1초 정지 → 페이지별 일정 속도로 왼쪽 이동 → 마지막 글자가 오른쪽 끝에 맞으면 1초 정지 → 다음 페이지** 순서입니다. 마지막 글자가 왼쪽 밖으로 사라질 때까지 밀지 않습니다. 짧은 문장은 가운데에 2초 고정합니다. 느린 일정 속도를 유지하므로 긴 메뉴와 원산지는 8초보다 오래 걸립니다.

알레르기는 `소고기무국 (5.6.16)`처럼 나이스 원본의 번호만 음식명 옆에 표시합니다. 이름 설명을 추가하지 않으며 원본 숫자를 보존합니다. 별도의 알레르기 페이지는 없습니다.

글자는 Noto Sans KR Medium을 18픽셀에서 직접 도트로 만든 Sangam18을 사용합니다. 기존 16픽셀 한글보다 조금 크게 표시하며, 가로·세로를 따로 늘리지 않습니다. 시계도 같은 글꼴이며 숫자는 11픽셀, 기호는 5픽셀, 공백은 7픽셀 칸을 사용해 콜론이 깜빡여도 숫자 위치가 움직이지 않습니다. 한 줄 가운데 정렬과 256×32 기준을 유지합니다.

1·3·5·7·9페이지는 초록색(RGB 0,255,0), 2·4·6·8페이지는 노란색(255,255,0)입니다. 위로 전환하는 동안에도 두 페이지는 각자의 색을 유지합니다.

원산지는 같은 표기끼리 묶습니다. 예: `쌀: 국내산 / 배추: 국내산 / 김치: 중국산` → `국내산: 쌀, 배추 / 중국산: 김치`. `국내산, 호주산`, `국내산(육우)`, `러시아산(국내가공)`처럼 여러 국가나 설명이 붙은 표기는 그대로 별도 묶음으로 유지합니다. 해석되지 않은 항목도 버리지 않고 뒤에 표시합니다.

화면 로직은 `screen_renderer.h`, 속도·정지·전환 시간은 `display_sequence.h`에 있습니다. `display_checks.h`는 타이밍·정지 위치·원산지 묶음 검사를 포함합니다. 앞으로 수정본은 버전이 표시된 전체 소스 ZIP으로 함께 보관합니다.

Wi-Fi를 바꿀 때는 **부팅 완료 후 BOOT 버튼을 5초간** 누릅니다. 연결 실패가 1분 지속되어도 설정용 Wi-Fi가 나타납니다.
